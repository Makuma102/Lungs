...
<tbody>{row("Small 3D U-Net (ours)", O, True)}{row("HU-threshold baseline", T)}{row("Empty prediction (control)", E)}</tbody></table>"""
    table_ctrl = f"""
...
<tr><td>Ours vs. expert label</td><td>{ci(O['dice'])}</td><td>main result</td></tr>
...
<tr><td>Empty prediction</td><td>{ci(E['dice'])}</td><td>must be exactly 0</td></tr>
    ...
    pc_rows = "".join(
        ...
        f"<td>{r['ours']['hd95']:.1f}</td><td>{r['ours']['tp']}/{r['ours']['tp'] + r['ours']['fn']}</td><td>{r['ours']['fp']}</td><td>{r['ceiling_dice']:.3f}</td></tr>"
        for r in per_case)
    table_cases = f"""
<table class="small"><caption><b>Table 3.</b> Per-patient results (test split), sorted by Dice. Volumes in mL; HD95 in mm (&infin; when nothing was predicted).</caption>
<thead><tr><th>Case</th><th>GT vol.</th><th>Pred. vol.</th><th>Dice</th><th>HD95</th><th>Lesions found</th><th>FP</th><th>Ceiling</th></tr></thead><tbody>{pc_rows}</tbody></table>"""
    ...
    for c, r in recon.items():
        ...
        rec_rows += (f"<tr><td>{c}</td><td>{lv / 1000:.2f}</td><td>{sum(k.startswith('lung_') for k in st)}</td>"
                     ...
                     f"<td>{aw.get('max_generation', '–')}</td><td>{st.get('vessels', {}).get('volume_ml', float('nan')):.0f}</td>"
                     f"<td>{r.get('tumor_gt', {}).get('lobe', '–').replace('lung_', '').replace('_', ' ')}</td>"
                     f"<td>{r.get('pred_metrics', {}).get('dice', float('nan')):.3f}</td></tr>")
    table_recon = f"""
<table class="small"><caption><b>Table 4.</b> Anatomy reconstructions. Lung volume in L; airway and vessel volumes in mL; airway graph from the pruned centreline (spurs &lt; 5 mm removed).</caption>
<thead><tr><th>Case</th><th>Lung</th><th>Lobes</th><th>Airway</th><th>Branch pts</th><th>Endpoints</th><th>Max gen.</th><th>Vessels</th><th>Tumor lobe</th><th>U-Net Dice</th></tr></thead><tbody>{rec_rows}</tbody></table>""" if recon else ""

    phantom = ""
    if ph:
        t = ph["test"]
        phantom = (f"On the synthetic phantom used for development (Appendix A) the 2D model reached lung Dice {t['lung_dice']['mean']:.3f} "
...
<p><b>Purpose.</b> We describe an open, fully reproducible pipeline that segments lung tumors on chest CT with a deliberately small 3D U-Net and turns each scan into an anatomy-level 3D model (five lobes, airway tree with centreline, pulmonary vessels and tumor). We report results with checks that catch <i>false success</i>. The whole thing runs on a 4-core CPU.
...
<b>Results.</b> On the test split the model reached tumor Dice {ci(O['dice'])}, HD95 {ci(O['hd95'], 1)} mm and lesion-level sensitivity {O['lesion_sensitivity']:.2f} with {O['fp_per_scan']:.2f} false-positive components per scan. It detected the tumor in {n_det} of {S['n_test']} patients. The threshold baseline scored Dice {ci(T['dice'])}; the shuffled-label control scored {ci(S['shuffled_gt_dice'])}; the resampling ceiling was {ci(S['resampling_ceiling_dice'])}.
<b>Conclusion.</b> A small model trained for about {sum(r['sec'] for r in tr['log'])/3600:.1f} CPU-hours gives a clear, honestly bounded baseline. It is well below published GPU-trained systems (about 0.69 Dice for nnU-Net). We release code, tests, the 3D viewer and every number in this report as machine-generated files. This is a research prototype and not a diagnostic device.</p>
</section>
...
{table_ctrl}
<figure>{img(os.path.join('results/msd', 'fig_dice_per_case.png'))}<figcaption><b>Figure 1.</b> Per-patient tumor Dice on the test split for our model and the threshold baseline; black ticks mark the resampling ceiling.</figcaption></figure>
<div class="row"><figure class="half">{img(os.path.join('results/msd', 'fig_volume.png'))}<figcaption><b>Figure 2.</b> Predicted vs. expert tumor volume (symmetric log axes).</figcaption></figure>
...
<figure>{img(os.path.join('results/msd', 'fig_qualitative.png'))}<figcaption><b>Figure 4.</b> Best, median and worst test patients (axial slice with the largest tumor area). Orange: expert; cyan: 3D U-Net.</figcaption></figure>
{table_cases}
<h3>4.1 Anatomy-level reconstruction</h3>
<p>Figure 5 shows reconstructions from real CT. Lobes, airway tree with centreline and endpoints, vessels and tumor are all in patient RAS coordinates, so they can be loaded straight into 3D Slicer or the web viewer. Table 4 summarises the derived anatomy.</p>
{table_recon}
<div class="row">{figs_recon}</div>
<p class="figcap"><b>Figure 5.</b> Anatomy-level reconstructions (anterior view; patient right on the viewer's left). Lobes are translucent, airway green with centreline, tumor orange (expert) and yellow wireframe (3D U-Net).</p>
<p>{phantom}</p></section>

<section><h2>5&nbsp; Discussion</h2>
<p><b>What the numbers mean.</b> Our tumor Dice is well below the about 0.69 that nnU-Net reports on this task <a href="#r2">[2]</a>. That is expected from a model with about {tr['params']/1e6:.0f}M parameters trained for {args['epochs']} short CPU epochs, compared with 1000 GPU epochs and five-fold ensembling. We do not claim state-of-the-art accuracy. The value of this work is a transparent, bounded baseline. The controls show the score is real (shuffled labels near 0), non-trivial (above the threshold rule) and limited partly by our own resampling (the ceiling is below 1 for small tumors).</p>
<p><b>Failure modes.</b> Most errors are on small tumors, where a few voxels of boundary disagreement dominate Dice, and on tumors abutting the mediastinum or chest wall, where the intensity contrast with neighbouring soft tissue is low (Fig. 4, worst case).</p>
